...
- **Block-wise processing**: splits input FASTQ files into configurable chunks (default 256 MB) to keep memory usage stable.
...
## Command-line usage
Use `main.py` to run either compressor. The `--compressor` flag selects the lossy or lossless pipeline, and `--mode` toggles between compression and decompression.
...
- **Lossless compression**
  ...
  ```bash
  python main.py --compressor Lossless --input_path input/sample.fastq --output_path output --mode c
  ```

- **Lossy compression (quantized qualities)**

  ```bash
  python main.py --compressor Lossy --input_path input/sample.fastq --output_path output --mode compress --save True
  ```

- **Decompression**

  ```bash
  python main.py --compressor Lossless --input_path output/sample --output_path output --mode d
  ```
...
```bash
python main.py --compressor "Lossless" --input_path "input/SRR554369.fastq" --output_path "output" --mode "c"
```

## Notes
- Default block size is 256 MB; adjust `block_size` in `Lossy.py` or `LossLess.py` if you need different memory characteristics.
