...
def _decompress_content(content, output_path, lpaq8_path, save, block_count,
    ...
    while True:
        ...
        # 显式删除大对象以释放内存
        del id_regex_data, id_tokens_data, g_prime_data, q_prime_data
        del id_block, g_prime, q_prime
        import gc
        gc.collect()
...
if __name__ == '__main__':
    ...
    if Debug:
        # 手动debug
        fastq_path = "/media/compress/新加卷1/test/SRR14139158_1.fastq"
        output_path = f"{os.getcwd()}/output"
        compressed_path = f"{os.getcwd()}/output/SRR14139158_1"
        # main("compress", fastq_path, output_path, lpaq8_path, False, None)
